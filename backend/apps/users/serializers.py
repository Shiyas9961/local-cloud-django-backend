...
class UserSerializer(serializers.ModelSerializer):
    """User serializer."""
    avatar_file = serializers.FileField(required=False, write_only=True, allow_null=True)
    ...
    def update(self, instance, validated_data):
        ...
        if avatar_file:
            try:
                ...
                avatar_url = upload_and_get_url(file_bytes=avatar_file.read(),folder_name="avatars", file_name=final_filename)
    ...
    def create(self, validated_data):
        ...
        with transaction.atomic():
            ...
            if avatar_file:
                upload_user_avatar_task.delay(user.id, avatar_file.name, avatar_file.read())

        transaction.on_commit(lambda: send_html_email.delay(
